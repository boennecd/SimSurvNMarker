...
instantaneous rate, $h(t\mid \vec u)$. $\vec m(t)$ and $\vec b(t)$ are 
...
when a first polynomial is used and $\vec\alpha = \vec 0$. $X_{ij}$ and 
$\vec z_i$ are individual specific known covariates.

The package is not too well documented at this point. Instead we provide an 
example of how to use the package here. 

## Example
We start by assigning all the variables that we will pass to the function
in the package.
...
individual. The dashed curve is the mean of $\vec\mu(s, \vec 0)$, the 
...
* Implement $X_{ij}\vec\beta$.
* Different basis for fixed time-varying effect and the random time-varying 
  effect in $\vec\mu$. Typically, one might one a more flexible fixed 
  effect. 
* Allow for derivatives of $\vec\mu$ in the hazard.  
* Handle missing components, e.g. no $\vec\delta^\top\vec z_i$. 
